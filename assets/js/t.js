var browserball = (function () {
    ...
    var l = {
        ...
        velocity: {x: 0, y: 0},
        inside: function (u, v) {
            return (this.offset.x >= Math.sqrt((this.x - u) * (this.x - u) + (this.y - v) * (this.y - v)))
        }
    };
    ...
    var o = {
        x0: Infinity, y0: Infinity, update: function () {
            var x, w, v = o.x0, A = o.y0, z;
    ...
    var e = {
        list: [], corners: [], add: function (u) {
            ...
            var E = [{x1: z.x1, y1: z.y1, x2: z.x2, y2: z.y1}, {x1: z.x2, y1: z.y1, x2: z.x2, y2: z.y2}, {
                x1: z.x2,
                y1: z.y2,
                x2: z.x1,
                y2: z.y2
            }, {x1: z.x1, y1: z.y2, x2: z.x1, y2: z.y1}], C = [{x1: y.x1, y1: y.y1, x2: y.x2, y2: y.y1}, {
                x1: y.x2,
                y1: y.y1,
                x2: y.x2,
                y2: y.y2
            }, {x1: y.x2, y1: y.y2, x2: y.x1, y2: y.y2}, {x1: y.x1, y1: y.y2, x2: y.x1, y2: y.y1}], A = null;
            var v, u, x, w, D, B;
            for (v = 0; v < 4; v++) {
                for (u = (v + 1) % 4, x = 0; x < 2; u = (u + 2) % 4, x++) {
                    ...
                    if (A && !e.pInsideAny(A.x, A.y)) {
                        ...
                        B = 0;
                        if (E[v].x1 == E[v].x2) {
                            D = (E[v].y1 < E[v].y2) ? 1 : -1, B = (C[u].x1 < C[u].x2) ? -1 : 1
                        } else {
                            D = (C[u].y1 < C[u].y2) ? 1 : -1, B = (E[v].x1 < E[v].x2) ? -1 : 1
                        }
            ...
            var z = {x1: w.x1, y1: w.y1, x2: w.x2, y2: w.y2}, y = {x1: v.x1, y1: v.y1, x2: v.x2, y2: v.y2}, u = null, x;
            if (z.x1 > z.x2) {
                x = z.x1;
                z.x1 = z.x2;
                z.x2 = x
    ...
    var h = function () {
        ...
        return {
            down: function (z) {
                var A = z.target.ownerDocument.defaultView, w = (A.screenX - o.x0) + z.clientX, B = (A.screenY - o.y0) + z.clientY;
                if (l.inside(w, B)) {
                    l.dragging = true;
                    l.rotation = 0;
    ...
    var n = function (w) {
        var x = w.target.defaultView || w.target, u = e.list[x.quad_ref], v = u.canvas;
    ...
    var r = function (y) {
        ...
        for (var v = 0, u = e.list.length; v < u; v++) {
            ...
            if (e.list[v].x1 != x.screenX - o.x0 || e.list[v].y1 != x.screenY - o.y0) {
                w = true
            }
        }
        if (w) {
    ...
    var g = function (y) {
        var x = "" + (window.screenY + 100), w = "" + (window.screenX - 200), u = "300", v = "300";
        window.open("child.html", "w" + e.list.length, "location=no,status=no,menubar=no,toolbar=no,scrollbars=no,status=no,width=" + v + ",height=" + u + ",left=" + w + ",top=" + x)
    };
    var m = function () {
        l.dragging = true;
        l.rotation = 0;
    ...
    var t = function () {
        var O = [], C = 1, A = 0;
        O.push(l.w - e.sInside(l.x - l.offset.x, l.y - l.offset.y, l.x + l.offset.x, l.y - l.offset.y, e.list.slice(0)));
        O.push(l.h - e.sInside(l.x + l.offset.x, l.y - l.offset.y, l.x + l.offset.x, l.y + l.offset.y, e.list.slice(0)));
        O.push(l.w - e.sInside(l.x - l.offset.x, l.y + l.offset.y, l.x + l.offset.x, l.y + l.offset.y, e.list.slice(0)));
        O.push(l.h - e.sInside(l.x - l.offset.x, l.y - l.offset.y, l.x - l.offset.x, l.y + l.offset.y, e.list.slice(0)));
        if ((!!O[0] || !!O[1] || !!O[2] || !!O[3])) {
            var J = 0, Q, M, v, N, I = 0;
            ...
            if (J && Q == C) {
                ...
                l.y -= Math.round(((J * l.velocity.y) / l.velocity.x)) * (l.velocity.y < 0 ? -1 : 1);
                ...
                if (J && Q == A) {
                    if (l.velocity.y > 1) {
                        l.x -= Math.round(((J * l.velocity.x) / l.velocity.y)) * (l.velocity.x < 0 ? -1 : 1)
                    }
                    ...
                    var u, D = Number.POSITIVE_INFINITY, S, T, P = -1, G = e.corners;
                    ...
                    if (P >= 0 && I != 3) {
                        var R = G[P].dx > 0 ? l.x > G[P].x : l.x < G[P].x, H = G[P].dy > 0 ? l.y > G[P].y : l.y < G[P].y, w;
                        if (((R && !H) || (H && !R))) {
                            if (R) {
                                w = l.radius - Math.abs(l.y - G[P].y);
                                if (l.velocity.y > 1) {
                                    l.x -= Math.round(((w * l.velocity.x) / l.velocity.y)) * (l.velocity.x < 0 ? -1 : 1)
                                }
                                ...
                                l.y -= Math.round(((w * l.velocity.y) / l.velocity.x)) * (l.velocity.y < 0 ? -1 : 1);
                                l.velocity.x = -l.velocity.x * a;
                                l.velocity.y = l.velocity.y * c;
                                l.rotation = l.velocity.y * 0.015
                            }
                            ...
                            if (D < l.radius) {
                                var F = l.velocity.x, E = l.velocity.y, B, z;
                                ...
                                l.velocity.y = B == -1 && z == -1 ? F * a * -G[P].dy : E * a * z;
    ...
    return {
        init: function () {
            ...
            var u = this.quad_ref, v = e.remove(u);
